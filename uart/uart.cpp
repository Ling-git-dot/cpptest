...
{
    //fd = open(port, O_RDWR|O_NOCTTY|O_NDELAY); //阻塞模式
    //fd = open(port, O_RDWR | O_NONBLOCK | O_NOCTTY); //非阻塞模式
    //fd = open(uart_name, O_RDWR | O_NOCTTY); //阻塞模式
    fd = open(port, O_RDWR | O_NONBLOCK | O_NOCTTY); //非阻塞模式
    //fd = open(port, O_RDWR | O_NOCTTY); //阻塞模式
    if (FALSE == fd)
    {
...
*              speed:串口速度
*	             flow_ctrl:数据流控制
*	             databits:数据位，取值为 7 或者 8
*	             stopbits:停止位，取值为 1 或者 2
